...
export const send = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    const vars: Record<string, string> = { paciente: patient.name, clinica: settings?.clinicName || 'Clinica Dr Marcelo', data: schedule ? formatDateBR(schedule.date) : '', horario: schedule?.startTime || '', profissional: professional?.name || '', sala: room?.name || '', telefone_clinica: settings?.phone || '', dica: '', regras: `Cancelamentos com pelo menos ${settings?.cancellationNoticeHours ?? 2}h de antecedência permitem reposição conforme a política da clínica.` }
