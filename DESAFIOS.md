...
- **Publicação**: Convex `exuberant-guanaco-180` atualizado. Deploy Vercel `dpl_FqdnFkENnrNGGixsVvAiKgumAvX6` (READY), aliases `https://altar-fisio.vercel.app` e `https://clinicadrmarcelo.vercel.app`. Endpoints públicos (`/`, `/sw.js`) e consulta remota `clinic:getSettings` responderam com HTTP 200.
