...
export function PatientWaitlist({ portalToken, onChangeSlot, bookingEnabled = true }: { bookingEnabled?: boolean; portalToken: string; onChangeSlot: (creditId: Id<'replacementCredits'>) => void }) {
  ...
  return <section aria-label="Minhas filas de espera" className="space-y-3">
    ...
    {visibleEntries.map(e => <div key={e._id} className="rounded-2xl border p-4 space-y-2">
      ...
      {e.status === 'waiting' && <div className="flex flex-wrap gap-2">
        <Button hidden={!bookingEnabled} style={bookingEnabled ? undefined : { display: 'none' }} size="sm" variant="outline" disabled={!!busy} onClick={() => onChangeSlot(e.creditId)}>Trocar horário</Button>
